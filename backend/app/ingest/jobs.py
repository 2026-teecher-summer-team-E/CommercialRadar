...
def ingest_seoul_business(db: Session | None = None) -> IngestionRun:
    ...
    두 서비스의 최신 분기를 각각 탐색·수신해 (TRDAR_CD, SVC_INDUTY_CD) 기준으로
    메모리 내 병합 후 upsert한다. 두 서비스의 최신 분기가 다를 수 있으며
    (실측: 추정매출 20254, 점포 20261), year_quarter는 추정매출 분기를 우선한다.
    seoul_commercial 선행 완료 필요 (commercial_district_id 조인).
    """
    owns_session = db is None
    db = db or SessionLocal()
    ...
    try:
        ...
        # 추정매출: 최신 분기 탐색 + 수신
        with SeoulClient(_SVC_SALES) as client:
            selng_quarter = client.find_latest_quarter()
            logger.info("[%s] 추정매출 최신 분기: %s", source, selng_quarter)
            selng_raws = list(client.iter_rows(quarter_filter=selng_quarter))

        # 점포: 최신 분기 탐색 + 수신
        with SeoulClient(_SVC_STORE) as client:
            stor_quarter = client.find_latest_quarter()
            logger.info("[%s] 점포 최신 분기: %s", source, stor_quarter)
            stor_raws = list(client.iter_rows(quarter_filter=stor_quarter))

        fetched = len(selng_raws) + len(stor_raws)

        # 각 소스를 (TRDAR_CD, SVC_INDUTY_CD) 인덱스로 빌드
        selng_index, selng_failed = business_transformer.build_selng_index(selng_raws)
        stor_index, stor_failed = business_transformer.build_stor_index(stor_raws)
        failed = selng_failed + stor_failed
        ...
        db.commit()
        logger.info(
            "인제스천 완료 [%s]: fetched=%d merged=%d upserted=%d failed=%d",
            source, fetched, len(merged_rows), upserted, failed,
        )
        return run
