"""category_search_trend upsert loader."""
...
def upsert_batch(db: Session, rows: list[dict]) -> int:
    if not rows:
        return 0
    stmt = insert(CategorySearchTrend).values([_to_insert_values(r) for r in rows])
    stmt = stmt.on_conflict_do_update(
        constraint="uq_category_trend_name_source_period",
        set_={
...
def upsert_all(db: Session, rows: list[dict]) -> int:
    ...
    for start in range(0, len(rows), BATCH_SIZE):
        ...
        try:
            ...
            db.rollback()
            logger.exception("category_search_trend upsert 실패 (batch start=%d)", start)
            raise
    return total
