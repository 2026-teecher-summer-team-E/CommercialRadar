"""Load 단계: 언피벗된 유동인구 dict를 population_heatmap에 멱등 upsert.

(commercial_district_id, dimension, slot) UNIQUE 제약(uq_pop_heatmap_cd_dim_slot)을
conflict 타겟으로 사용해 재실행 시 중복 없이 avg_population을 갱신한다.
"""
...
def upsert_batch(db: Session, rows: list[dict]) -> int:
    ...
    stmt = insert(PopulationHeatmap).values([_to_insert_values(r) for r in rows])
    stmt = stmt.on_conflict_do_update(
        constraint="uq_pop_heatmap_cd_dim_slot",
        set_={
...
def upsert_all(db: Session, rows: list[dict]) -> int:
    ...
    for start in range(0, len(rows), BATCH_SIZE):
        ...
        try:
            ...
            db.rollback()
            logger.exception(
                "유동인구 배치 upsert 실패 (start=%d, size=%d)", start, len(batch)
            )
            raise
    return total
